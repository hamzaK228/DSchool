...
export async function GET(request: NextRequest) {
  ...
  if (profile?.role === "teacher" && profile?.class_id) {
    const { data } = await adminClient.from("exam_submissions").select("*, profiles(full_name)").eq("student_id", user.id);
    return NextResponse.json(data || []);
  } else {
    const { data } = await adminClient.from("exam_submissions").select("*").eq("student_id", user.id);
    return NextResponse.json(data || []);
  }
}
